...
func main() {
	...
	poolAddress := solana.MustPublicKeyFromBase58("AUUXZxw1uizZZqna65fdM1ct1f2GWqkivSGyqm55mWQK")

	// ammBuild := amm_v3.NewSwapV2InstructionBuilder().SetAmount(amount uint64)
	client := raydium.NewCpmmClient(wallet)

	tx, err := client.Swap(ctx, 1078, poolAddress, false, 0.05)
	if err != nil {
